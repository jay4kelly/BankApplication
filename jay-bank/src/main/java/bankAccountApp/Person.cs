...
public class Person {
	...
	public Person(String newName, char newGender, int newAge, float newWeight, float newHeight, String newHairColor,
			String newEyeColor, String newEmail)  {
		name = newName;
		gender = newGender;
		age = newAge;
		weight = newWeight;
	...
	public Person(String accountHolder) {
		Scanner scan = null;
		try {
	 ...
	 * @throws Exception
	 */
	private boolean validateGender(char value)  {
		if ((value == 'M') || (value == 'F')|| (value == 'm') || (value == 'f' ) || (value =='o') || (value =='O'))
			return true;
		else
